...
void Set_ImGui_MousePos(float x, float y, int orientation) {
    ImGuiIO &io = ImGui::GetIO();
    float physic_w = (float)ANativeWindow_getWidth(g_Window);
    float physic_h = (float)ANativeWindow_getHeight(g_Window);


    if (physic_w > physic_h) {
        float tmp = physic_w;
        physic_w = physic_h;
        physic_h = tmp;
    }
...
int32_t ImGui_ImplAndroid_HandleInputEvent(motion_event *motion_event, int orientation) {
    ImGuiIO &io = ImGui::GetIO();
    int32_t event_type = AINPUT_EVENT_TYPE_MOTION;
    switch (event_type) {
        case AINPUT_EVENT_TYPE_MOTION: {
            int32_t event_action = motion_event->action;
            ...
            switch (event_action) {
                ...
                case AMOTION_EVENT_ACTION_UP:
                    ...
                    Set_ImGui_MousePos(motion_event->x, motion_event->y, orientation);
                    break;
